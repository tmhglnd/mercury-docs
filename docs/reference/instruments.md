...
Use the `noise` instrument if all you want is just different types of noises! sssssshh, kkkggggg, brrrrr, ffffrrr. The instrument has the common noise-generators: `white` and `pink`, but it also has a few other types: `brownian`, `lofi`, `crackle` and `dust`. All other common instrument functions (like time, shape, gain, pan, fx, etc) can be applied on the noise. Create the instrument with the following syntax:

```js
new noise <noise-type>
```

You can choose from the following noise-types:

- `white` - a random signal with an equal intensity across the frequency spectrum. Sounds very bright/harsh.
- `pink` - a random signal where each octave contains an equal amount of energy. Sounds like the sea or a waterfall
- `brown` - noise generated by a random walk (or also called brownian motion). Sounds more dark and has a rumble.
- `lofi` - low fidelity noise, generated by downsampling the noise signal. Sounds more 8bit.
- `crackle` - some random clicks, pops and cracks. Sounds like vinyl noise.
- `dust` - random impulses (clicks). Sounds more crispy then the vinyl noise.

### density

Control the density of the noise-types: brownian, lofi, crackle and dust. The density controls the frequency at which new noise values in the signal are generated. The higher the density (close to 1), the more the noise starts to sound like white noise. The lower the density (towards 0), the more darker/sparse the noise will be. Depending on the noise-type the sound will be different.

**arguments**
- `Float+(List)` -> the density between 0 and 1 (default = 0.2)
