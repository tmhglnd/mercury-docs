...
Set the random seed as an integer for the psuedo-random number generators used in all `list-function`s across the environment. Setting the seed to a fixed integer will help make sure random values keep the same sequence every time you re-evaluate the code. A second optional argument resets the seed every n-bars, which can be useful for random arguments used outside list generation, such as `pan(random)` or `beat(0.5)`
